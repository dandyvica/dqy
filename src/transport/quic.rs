...
use super::{
    ...
    endpoint::EndPoint,
    network::{unspec, AsyncMessenger, Protocol},
};
use super::{NetworkStat, TransportOptions, TransportProtocol};
use crate::error::{Dns, Error, Network, QuicError, Result};
...
impl QuicProtocol {
    pub async fn new(trp_options: &TransportOptions) -> Result<Self> {
        ...
        // let host = &trp_options.endpoint.server;
...
impl AsyncMessenger for QuicProtocol {
    ...
    async fn arecv(&mut self, buffer: &mut [u8]) -> Result<usize> {
        ...
        println!("about to read {} bytes in the TCP stream {:?}", length, self.handle.1);
